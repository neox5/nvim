return {
	...
	config = function()
		...
		-- Binary checks
		local function check_binary(binary, install_hint)
			if vim.fn.executable(binary) == 0 then
				vim.notify(string.format("DAP binary not found: %s\n%s", binary, install_hint), vim.log.levels.WARN, { title = "DAP" })
				return false
			end
			return true
		end

		check_binary("dlv", "Install with: go install github.com/go-delve/delve/cmd/dlv@latest")
		check_binary("node", "Install from: https://nodejs.org")
		check_binary("js-debug-adapter", "Install with: npm install -g @vscode/js-debug")
		...
		dap_virtual_text.setup({
			...
			display_callback = function(variable, buf, stackframe, node, options)
				...
				if #value > 50 then value = string.sub(value, 1, 50) .. "..." end
		...
		local function toggle_dap_virtual_text() vim.cmd("DapVirtualTextToggle") end
		...
		vim.api.nvim_create_autocmd("FileType", {
			...
			callback = function(event)
				vim.keymap.set("n", "<leader>td", function() require("dap-go").debug_test() end, { buffer = event.buf, desc = "Debug test" })

				vim.keymap.set("n", "<leader>tD", function() require("dap-go").debug_last_test() end, { buffer = event.buf, desc = "Debug last test" })
			end,
		})
